...
const Banrisul = require('../../boleto/bancos/banrisul');

const Gerador = require('../../boleto/gerador-de-boleto');
const GeradorCarne = require('../../boleto/gerador-de-boleto-carne');
...
module.exports.GeradorCarne = GeradorCarne;
//Baixar depois:
...
var Pagador = (function () {
	...
	Pagador.prototype.getNomeSomente = function () {
...
var Beneficiario = (function () {
	...
	Beneficiario.prototype.getIdentificacaoCompleta = function () {
        const identificacao = this.getIdentificacao();
        const endereco = this.getEndereco() ? this.getEndereco().getEnderecoCompleto() : '';
        return [identificacao, endereco].filter(Boolean).join('\n');
    };
	return Beneficiario;
})();
...
var Datas = (function () {
	...
	Datas.prototype.comDocumento = function (_documento,locate = 'usa') {
		...
		if (!validarData(_documento)) {
			throw new Error('O ano do documento deve ser maior que 1997 e menor que 2999');
		}
...
var Boleto = (function () {
	...
	Boleto.prototype.comCodigoBarras = function (_codigoBarras) {
	...
	Boleto.prototype.comIdUnico = function () {
	...
	Boleto.prototype.getValorCobrado = function () {
		return this._valorCobrado || 0;
	};

	Boleto.prototype.comValorCobrado = function (_valorCobrado) {
		this._valorCobrado = _valorCobrado;
		return this;
	};


	Boleto.prototype.getValorCobradoFormatadoBRL = function () {
		if (!this.getValorCobrado()) {
			return '';
		}

		return formatarBRL(formatarValor(this.getValorCobrado()));
	};

	Boleto.prototype.getValorMoraMultaJuros = function () {
		return this._valorMoraMultaJuros || 0;
	};

	Boleto.prototype.comValorMoraMultaJuros = function (_valorMoraMultaJuros) {
		this._valorMoraMultaJuros = _valorMoraMultaJuros;
		return this;
	};


	Boleto.prototype.getValorMoraMultaJurosFormatadoBRL = function () {
		if (!this.getValorMoraMultaJuros()) {
			return '';
		}

		return formatarBRL(formatarValor(this.getValorMoraMultaJuros()));
	};



	
	Boleto.prototype.getInformativo = function () {
		return this._informativo || [];
	};

	Boleto.prototype.comInformativo = function (_informativo) {
		if (arguments.length > 1) {
			_informativo = Array.prototype.slice.call(arguments, 0);
		}

		if (typeof _informativo === 'string') {
			_informativo = [_informativo];
		}

		if (_informativo.length > 5) {
			throw new Error('Máximo de cinco instruções permitidas');
		}

		this._informativo = _informativo;
		return this;
	};
