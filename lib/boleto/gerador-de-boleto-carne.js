  ...
  gerarPDF(args) {
    return new Promise(async (resolve) => {
      ...
      for (const [indice, boleto] of boletos.entries()) {
        ...
        ajusteYAtual += 255; 
        //if (contador === 3 && i < boletos.length - 1){
          if (contador === 3) {
         
            pdf.addPage(); // Adiciona nova página
            pdf.undash(); // Remove o estilo pontilhado para a próxima página
            contador = 1; // Reinicia o contador
            ajusteYAtual = args.ajusteY; // Reinicia o deslocamento vertical
          } else {
            pdf
              .undash() // Garante linha contínua
              .moveTo(args.ajusteX + 27, args.ajusteY + linhaSeparadora)
              .lineTo(args.ajusteX + 572, args.ajusteY + linhaSeparadora)
              .stroke(args.corDoLayout);
