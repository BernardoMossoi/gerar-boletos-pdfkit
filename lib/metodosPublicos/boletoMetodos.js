  ...
  gerarBoleto() {
    ...
    for (const boleto of this.boletos) {
      ...
      const boletoInfo = Boleto.Boleto.novoBoleto()
        ...
        .comValorMoraMultaJuros(
          boleto.boleto.ValorMoraMultaJuros
            ? parseFloat(boleto.boleto.ValorMoraMultaJuros).toFixed(2)
            : 0
        )

        .comLocaisDePagamento(boleto.locaisDePagamento || [])
